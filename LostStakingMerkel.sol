...
contract LostStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    function updateMarkelTree(bytes32 _merkleRoot1, bytes32 _merkleRoot2, bytes32 _merkleRoot3, bytes32 _merkleRoot4) external onlyOwner(){
    ...
    function deposit(markelData[] memory _markelData) public payable returns(uint256[] memory){
        // require(msg.sender != stakingDestinationAddress, "Invalid address");
        // claimRewards(tokenIds);
        uint256[] memory _rarityValueArray;
        for (uint256 i; i < _markelData.length; i++) {

            string memory indexNum = Strings.toString(_markelData[i]._indexNum);

            bool matchSuccess = false;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
            uint256 _rarityValue = 0; 

            if(MerkleProof.verify(_markelData[i].merkleProof, merkleRoot1, keccak256(abi.encodePacked(indexNum)))){
                _rarityValue = 0;
            }else if(MerkleProof.verify(_markelData[i].merkleProof, merkleRoot2, keccak256(abi.encodePacked(indexNum)))){
                _rarityValue = 1;
            }else if(MerkleProof.verify(_markelData[i].merkleProof, merkleRoot3, keccak256(abi.encodePacked(indexNum)))){
                _rarityValue = 2;
            }else if(MerkleProof.verify(_markelData[i].merkleProof, merkleRoot4, keccak256(abi.encodePacked(indexNum)))){
                _rarityValue = 3;
            }else{
                require(matchSuccess, "Invalid merkel dat");
            }

            uint256 blockCur1 = block.timestamp;
            _depositBlocksTmp[msg.sender][_markelData[i]._indexNum] = blockCur1;
            _depositRarity[msg.sender][_markelData[i]._indexNum] = _rarityValue;
            
            IERC721(stakingDestinationAddress).safeTransferFrom(
                msg.sender,
                address(this),
                _markelData[i]._indexNum,
                ""
            );

            _deposits[msg.sender].add(_markelData[i]._indexNum);

        }

        return _rarityValueArray;
    }
